...
//    List<Person> selectPersonByNumber(String videoId, String number);
    ...
    // 根据学号查询
    @Select("select * from person where number=#{number}")
    ...
    // 更新人物信息
    @Update("update person set person_id=#{personId}, video_id=#{videoId}, number=#{number}, name=#{name}, investment=#{investment}, divorce=#{divorce}, listen=#{listen}, communication=#{communication}, play_phone_score=#{playPhoneScore}, play_laptop_score=#{playLaptopScore}, read_book_score=#{readBookScore}, raise_hand_score=#{raiseHandScore}, bow_score=#{bowScore}, lean_score=#{leanScore}, play_phone_num=#{playPhoneNum}, play_laptop_num=#{playLaptopNum}, read_book_num=#{readBookNum}, raise_hand_num=#{raiseHandNum}, bow_num=#{bowNum}, lean_num=#{leanNum} where person_id=#{personId}")
