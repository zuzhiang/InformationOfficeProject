...
def get_obj_info(obj_path, json_path):
    threshold = 0.15
    hand_file = open(os.path.join(obj_path, "hand.txt"), "r")
    laptop_file = open(os.path.join(obj_path, "laptop.txt"), "r")
    phone_file = open(os.path.join(obj_path, "phone.txt"), "r")
    book_file = open(os.path.join(obj_path, "book.txt"), "r")
    ...
    # 从pose.json文件里面读取数据
    f = open(os.path.join(json_path, "pose.json"), "r")
    ...
    for i in range(0, len(laptop)):  # 遍历每一行，即每个图片
        ...
        file_name = lst[-2] + os.sep + lst[-1]  # 如1/1127_1_1.jpg
        obj_dic = dict()  # 每张图片对应的字典
        # 包含物体检测信息的图像路径
        obj_dic.setdefault("address", os.path.join(obj_path.replace("txt", "img").replace(rtPath, ""), file_name))
        ...
        obj_dic.setdefault("time", person_num + str(int(name.split("_")[1]) * 100 + int(name.split("_")[2])))
        ...
        if float(hand_lst[0]) != 0. or float(hand_lst[1]) != 0. or float(hand_lst[2]) != 0. or float(
            ...
            for j in range(0, len(hand_lst) // 6 * 6, 6):  # 每一个手  
                ...
                if obj_dic["phone_exist"] == 1:
                    hand_bbox=(float(hand_lst[j]), float(hand_lst[j+1]), float(hand_lst[j+2]), float(hand_lst[j+3]))
                    for k in range(0, len(phone_lst) // 6 * 6, 6):  # 每一个手机
                        phone_bbox=(float(phone_lst[k]), float(phone_lst[k+1]), float(phone_lst[k+2]), float(phone_lst[k+3]))
                ...
                if obj_dic["book_exist"] == 1:
                    ...
                    for k in range(0, len(book_lst) // 6 * 6, 6):  # 每一本书
                        book_bbox=(float(book_lst[k]), float(book_lst[k+1]), float(book_lst[k+2]), float(book_lst[k+3]))
        ...
        #     obj_dic["phone_exist"] = 1
        #     for k in range(0, len(phone_lst) // 6 * 6, 6):  # 每一个手机
        #         if get_bool(hand_x1, hand_y1, hand_x2, hand_y2, float(phone_lst[k]), float(phone_lst[k + 1]),
        #                     float(phone_lst[k + 2]), float(phone_lst[k + 3]), "phone"):
        #             obj_dic["playphone_exist"] = 1
        #             print("图片%s中的人正在玩手机！" % file_name)
        #         # print("手机  ", file_name)
...
def get_pose_info(json_path):
    ...
    for key in json_dict.keys():  # json中的每一个键，即每一张图片
        ...
        pose_dic.setdefault("address", os.path.join(json_path.replace("json", "img").replace(rtPath, ""), key))
        ...
        pose_dic.setdefault("time", person_num + str(int(name.split("_")[1]) * 100 + int(name.split("_")[2])))
        ...
        # 举手状态判断
        th_angle = 40.0
        angle1 = get_angle(x[3], y[3], x[2], y[2], x[3], y[3], x[4], y[4])  # 节点顺序有要求：公共节点、节点1、公共节点、节点2
        angle2 = get_angle(x[6], y[6], x[5], y[5], x[6], y[6], x[7], y[7])
        h1 = abs((y[4] - y[2]) / (math.sqrt((x[2] - x[5]) ** 2 + (y[2] - y[5]) ** 2) + 1e-8))
        h2 = abs((y[5] - y[7]) / (math.sqrt((x[2] - x[5]) ** 2 + (y[2] - y[5]) ** 2) + 1e-8))
        w1 = abs(x[4] - (x[2] + x[5]) / 2) / abs(x[2] - x[5] + 1e-8)
        w2 = abs(x[7] - (x[2] + x[5]) / 2) / abs(x[2] - x[5] + 1e-8)
        # if angle1 < th_angle or angle2 < th_angle and h < 0.48:
        ...
        # 侧身状态判断
        """
        angle1 = get_angle(x[1], y[1], x[0], y[0], x[1], y[1], x[5], y[5])
        angle2 = get_angle(x[1], y[1], x[0], y[0], x[1], y[1], x[2], y[2])
        if (math.fabs(angle1 - angle2) > 80):
            dist = math.fabs(x[2] - x[5]) / (math.sqrt((x[0] - x[1]) ** 2 + (y[0] - y[1]) ** 2) + 1e-8)
            if (dist < 1.5):
                pose_dic["lean_exist"] = 1
                print("图片%s中的同学处于侧身状态" % key)
        """
        resnn1 = (x[14]-x[16])*(x[14]-x[16])+(y[14]-y[16])*(y[14]-y[16])
        resnn2 = (x[15] - x[17]) * (x[15] - x[17]) + (y[15] - y[17]) * (y[15] - y[17])
...
def faceRecognition(face_path):
    paths = glob.glob(os.path.join(face_path, "*.jpg"))
    # print("path: ",os.path.join(face_path,"*.jpg"))
    # print("len: ",len(paths))
    url = ""
    ...
    # sign = "accessKey=" + accessKey + "&nonceStr=" + nonceStr + "&timeStamp=" +timeStamp + "&accessSecret=" + accessSecret
    # print(sign)
    # m = hashlib.md5()
    # m.update(sign.encode("utf8"))
    # sign = m.hexdigest().upper()
    # print(sign)
    data = {"timeStamp": timeStamp, "accessKey": accessKey, "nonceStr": nonceStr, "sign": sign}
...
def get_person_name(img_path):
    ...
    img_person={ "1127_2_1.jpg":[学号，姓名],
                    ...}
    '''
    for cur_path in glob.glob(os.path.join(img_path, "*")):
        img_person = faceRecognition(cur_path)
        # print("img_person:\n",img_person)
        ...
        pic_lst, time_lst = glob.glob(os.path.join(cur_path.replace("face", "img"), "*.jpg")), []
        ...
        if stud_num != 0:
            face_dct.setdefault(stud_num, {"name": num_dct[stud_num][1], "images": pic_lst})
        else:
            face_dct.setdefault("-", {"name": "未识别", "images": pic_lst})
    return face_dct
...
if __name__ == '__main__':
    ...
    # obj_path = r"C:\Users\zuzhiang\Desktop\results\txt\1127"
    # json_path = r"C:\Users\zuzhiang\Desktop\results\pose\1127\json"

    root_path = sys.argv[1]
    obj_path = sys.argv[2]
    json_path = sys.argv[3]
    index = str(sys.argv[4])

    # 获取手机、电脑、手、书籍等物品，玩手机、玩电脑、读书等动作是否存在
    print("get_obj_info......")
    obj_lst = get_obj_info(obj_path, json_path)
    print("len(obj_lst): ",len(obj_lst))
    
    # 获取低头、举手、侧身等信息
    print("get_pose_info......")
    pose_lst = get_pose_info(json_path)
    ...
    face_dct = get_person_name(os.path.join(root_path, "results", "face", index))
    # print("face_dct:\n",face_dct)
    ...
    for key, value in face_dct.items():
        # 当前学号对应的字典
        person_dct = dict()
        object, pose = [], []
        f0, f1, f2 = 0, 0, 0
        ...
        person_dct.setdefault("name", value["name"])
        ...
        return_dct.setdefault(key, person_dct)
